 ...
 */
export async function runInstantQueries(queries: Record<string, string>): Promise<DataFrame[]> {
  ...
  return runPromQueries(targets, getDefaultTimeRange());
}
 ...
 */
export async function runRangeQuery(refId: string, expr: string, hours: number): Promise<DataFrame[]> {
  ...
  return runPromQueries([{ refId, expr, instant: false, range: true }], range);
}
