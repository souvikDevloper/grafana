...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	corepluginRegistry := coreplugin.ProvideCoreRegistry(tracer, azuremonitorService, cloudwatchService, graphiteService, influxdbService, lokiService, prometheusService, testdatasourceService, postgresService, mysqlService, mssqlService, grafanadsService, pyroscopeService, jaegerService)
	...
	migrationProxy, err := annotationsapi.ProvideMigrationProxy(cfg, userimplService)
	if err != nil {
		return nil, err
	}
	...
	folderContentsDeleter := apiregistry.ProvideFolderContentsDeleter(dBstore, libraryPanelService)
	folderAPIBuilder := folders.RegisterAPIService(cfg, featureToggles, apiserverService, folderPermissionsService, accessClient, registerer, resourceClient, zanzanaClient, eventualRestConfigProvider, folderContentsDeleter)
